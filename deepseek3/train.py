...
model = TinyDeepSeek(ModelConfig(vocab_size=256)).to(DEVICE)
...
for step in range(1, 5001):
    ...
    if step % 500 == 0: print(f"DeepSeek3 Adım {step}: Loss {loss.item():.4f}")

torch.save({"model": model.state_dict(), "cfg": model.cfg}, "tiny_deepseek_muzik_v2.pt")